...
Sin embargo, tiene algunos aspectos negativos. El tiempo de ejecución puede ser elevado, particularmente por dos factores.
Por un lado, la inicialización está dominada por la carga de varios diccionarios en en tablas \textit{hash}.
Por otro, debido al uso de la recursividad y al tratar explorar las posibilidades arrancando desde los extremos para finalmente comparar los resultados (identifier splitting: a study of two techniques).
Otra desventaja que posee es que tiende a realizar más divisiones que las esperadas, o como se conoce, hace oversplitting (an empirical comparison of...).
