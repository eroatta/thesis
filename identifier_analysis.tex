...
El proceso comienza dividiendo el token en hardwords, tomando como delimitadores a los caracteres especiales y la técnica de \textit{camel-casing}.
Hechos tanto el reemplazo de números y caracteres especiales, como la división inicial del token, comienza la evaluación de cada una de las hardwords.
En primera instancia, se verifica si la palabra candidata existe en alguna de las listas con las que trabaja el algoritmo. Si es así, el fragmento del token es considerado parte de la división y por lo tanto de la salida del proceso.
En caso contrario, al asumirse que está formado por múltiples palabras, se analiza en búsqueda de las softwords que lo componen.
Este análisis consiste en realizar dos búsquedas sobre cada hardword, quedándose con el mejor resultado entre ambas.
En la primera búsqueda se trata de obtener el prefijo más largo, de la palabra actual, que se encuentre en una lista. Luego se llama recursivamente, trabajando sobre el fragmento restante de la palabra original (prefijos y sufijos).
La segunda búsqueda es idéntica, excepto que se basa en el sufijo más largo.
Los resultados de ambas búsquedas se evalúan, y aquella que arroje la relación más alta entre la cantidad de softwords encontradas en listas vs. el total de palabras es elegida.
Si ni la búsqueda por prefijo o sufijo generan alguna palabra que se encuentre en alguna de las listas, el proceso se repite removiendo el primer caracter.
Estos caracteres removidos son agrupados y luego agregados al resultado. Si la primera palabra (softword) en el identificador dividido no está en una de las listas, los caracteres se van acoplando. Si lo está, se agregan como una nueva softword.
Al finalizar la ejecución del algoritmo, la salida que se obtiene es el identificador dividido, separando cada una de las softwords con espacios en blanco.

A continuación se presenta, en pseudocódigo, el algoritmo Greedy.
PSEUDOCODIGO

El algoritmo es independiente del conjunto de datos sobre el que se ejecuta, lo cual es una ventaja. Sin embargo, tiene algunos puntos negativos, como ser que el tiempo de ejecución puede ser elevado debido a dos factores: por un lado, la inicialización puede tornarse lenta ya que está dominada por la carga de varios diccionarios en "hash tables"; por otro, debido al uso de la recursividad y de explorar las posibilidades al arrancar de los extremos y finalmente comparar (identifier splitting: a study of two techniques). Otra desventaja que posee es que tiende a realizar más divisiones que las esperadas, o como se conoce, hace oversplitting (an empirical comparison of...).
Una ventaja de este algoritmo es que el mismo es independiente de los datos sobre los que se ejecuta.
